...
async def upload_stream(object_name: str,
    ...
    await asyncio.to_thread(app.minio_client.put_object,
            bucket_name=bucket,
...
def download_stream(object_name: str, bucket: str = OUTPUT_BUCKET):
    stat = app.minio_client.stat_object(bucket, object_name)
    fmt = MIME_TO_EXT.get(stat.content_type)
    respones = app.minio_client.get_object(bucket_name=bucket, object_name=object_name)
...
def delete_object(bucket: str, object_name: str):
    objects_to_delete = app.minio_client.list_objects(bucket, prefix=object_name, recursive=True)
    objects_to_delete = [x.object_name for x in objects_to_delete]
    for object in objects_to_delete:
        app.minio_client.remove_object(bucket, object)
    return {
        "bucket": bucket,
        "object": object_name
    }
...
def stream(video_id: str, file_path: str):
    ...
    """
    respones = app.minio_client.get_object(OUTPUT_BUCKET, f"{video_id}/{file_path}")
    return StreamingResponse(respones)

@app.get("/profile/{user_id}")
def profile_img(user_id: str):
    respones = app.minio_client.get_object(PROFILE_BUCKET, f"{user_id}.jpg")
    return StreamingResponse(respones)
