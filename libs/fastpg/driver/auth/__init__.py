...
def clear_text(reader: BinaryIO, writer: BinaryIO, passwd: str, init_pkt: bytes):
    writer.write(PasswordMessage.build({"password": [passwd]}))
    writer.flush()
    while True:
        pkt = sync_read_simple_packet(reader)
        if AuthenticationOk.matches(pkt):
            break
    wait(reader, "clear text authentication")
    return True

def sasl(reader: BinaryIO, writer: BinaryIO, passwd: str, init_pkt: bytes):
    ...
    wait(reader, "sasl authentication")
    return True
